...
# Log a not-found item to the report file
log_not_found() {
    local section="$1" item="$2"
    echo "[${section}] ${item}" >> "$NOT_FOUND_FILE"
}
...
# ── Not-found report ──────────────────────────────────────────────────────────
NOT_FOUND_FILE="${SCRIPT_DIR}/not_found.txt"
: > "$NOT_FOUND_FILE"   # truncate / create on every run
info "Items not found during installation will be logged to: ${NOT_FOUND_FILE}"
...
if [[ ${#_not_in_repos[@]} -gt 0 ]]; then
    ...
    warn "These have been saved to: ${NOT_FOUND_FILE}"
fi
...
if [[ "$is_laptop" =~ ^[Yy]$ ]]; then
    ...
    if ! pacman -S --noconfirm power-profiles-daemon; then
        log_not_found "pacman" "power-profiles-daemon"
        ask_on_error "Failed to install power-profiles-daemon."
    fi
fi
...
# ── Not-found report summary ──────────────────────────────────────────────────
echo
if [[ -s "$NOT_FOUND_FILE" ]]; then
    warn "═══ Some items could not be installed ═══"
    warn "The following were not found or failed — see: ${NOT_FOUND_FILE}"
    while IFS= read -r line; do
        echo -e "    ${RED}✗${RESET} ${line}"
    done < "$NOT_FOUND_FILE"
else
    success "All items installed — nothing to report in ${NOT_FOUND_FILE}."
fi
